...
def _low_rank_helper(mat, low_rank, suffix):
    ...
    if low_rank > 0:
        ...
        w_mat = tf.matmul(w_mat1, w_mat2, 'w_%s' % suffix)
    else:
...
def inference(x, conf):
    ...
    with tf.name_scope('overlap_avg'):
        ...
        y_average = tf.div(y_stitch_o, mask + 1e-8, name='y_average')
